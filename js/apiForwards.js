...
const getAllData = async () => {
    try {
        let res = await axios.get('http://localhost:3000/forwards');
...
document.addEventListener('submit', async (e) => {
    if(e.target === $form){
        ...
        if(!e.target.id.value){
            /* ---> CREATE <--- */
            try {
                let res = await axios('http://localhost:3000/forwards', {
                    method: 'POST',
                    headers: {
            ...
            /* ---> UPDATE <--- */
            try {
                let res = await axios(`http://localhost:3000/forwards/${e.target.id.value}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type':'application/json'
                    },
...
document.addEventListener('click', async (e) => {
    ...
    if(e.target.matches('.btnDelete')){
        ...
        if(conf){
            try {
                let res = await axios(`http://localhost:3000/forwards/${e.target.dataset.id}`, {
                    method: 'DELETE'
                });
                let data = await res.data;
                location.reload();
