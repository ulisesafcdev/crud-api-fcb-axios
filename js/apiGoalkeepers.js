...
const getAllData = async () => {
    try {
        let res = await axios.get('http://localhost:3000/goalkeepers');
        console.log(res);
...
document.addEventListener('submit', async (e) => {
    if (e.target === $form) {
        ...
        if (!e.target.id.value) {
            /* ---> CREATE <--- */
            try {
                let res = await axios('http://localhost:3000/goalkeepers', {
                    method: 'POST',
                    headers: {
            ...
            /* ---> DELETE <--- */
            try {
                let res = await axios(`http://localhost:3000/goalkeepers/${e.target.id.value}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json'
                    },
...
document.addEventListener('click', async (e) => {
    ...
    if(e.target.matches('.btnDelete')){
        ...
        if(conf){
            try {
                let res = await axios(`http://localhost:3000/goalkeepers/${e.target.dataset.id}`, {
                    method: 'DELETE',
                })
                let data = await res.data;
                location.reload();
